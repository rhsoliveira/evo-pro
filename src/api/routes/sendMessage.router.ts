...
export class MessageRouter extends RouterBroker {
  constructor(...guards: RequestHandler[]) {
    ...
    this.router
      ...
      .post(this.routerPath('sendPaymentOrder'), ...guards, async (req, res) => {
        const response = await this.dataValidate<SendPaymentOrderDto>({
          request: req,
          schema: paymentOrderMessageSchema,
          ClassRef: SendPaymentOrderDto,
          execute: (instance, data) => sendMessageController.sendPaymentOrder(instance, data),
        });

        return res.status(HttpStatus.CREATED).json(response);
      })

      .post(this.routerPath('sendMessageAnyType'), ...guards, async (req, res) => {
        const response = await this.dataValidate<SendMessageAnyType>({
          request: req,
          schema: sendMessageAnyTypeSchema,
          ClassRef: SendMessageAnyType,
          execute: (instance, data) => sendMessageController.sendMessageAnyType(instance, data),
        });
