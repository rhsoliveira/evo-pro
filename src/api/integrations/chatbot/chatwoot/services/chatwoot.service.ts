...
export class ChatwootService {
  ...
  // Media download timeout (60 seconds for large files)
  private readonly MEDIA_DOWNLOAD_TIMEOUT_MS = 60000;
  ...
  private async sendData(
    ...
    const config = {
      ...
      data: data,
      timeout: this.MEDIA_DOWNLOAD_TIMEOUT_MS, // Increased timeout for media uploads
    };
  ...
  public async sendAttachment(waInstance: any, number: string, media: any, caption?: string, options?: Options) {
    try {
      ...
      const data: SendMediaDto = {
        ...
        fileName: fileName,
        media: media,
        delay: 3000, // Increased delay for media to prevent timeout
        quoted: options?.quoted,
      };
  ...
  private getTypeMessage(msg: any) {
    const types = {
      ...
      buttonResponseMessage: msg.buttonResponseMessage,
      nativeFlowResponseMessage: msg.nativeFlowResponseMessage,
      viewOnceMessageV2:
  ...
  private getMessageContent(types: any) {
    ...
    if (typeKey === 'interactiveMessage') {
      ...
      const footerText = result?.footer?.text || '';
      const headerText = result?.header?.hasText ? result?.header?.text : '';
      const headerImage = result?.header?.hasImage ? '[Image]' : '';
      const headerVideo = result?.header?.hasVideo ? '[Video]' : '';
      const headerDocument = result?.header?.hasDocument ? '[Document]' : '';

      let formattedButton = '*Button Message:*\n\n';

      if (headerText) {
        formattedButton += `*${headerText}*\n\n`;
      }
      if (headerImage || headerVideo || headerDocument) {
        formattedButton += `${headerImage || headerVideo || headerDocument}\n\n`;
      }
      if (bodyText) {
        formattedButton += `${bodyText}\n`;
      }
      if (footerText) {
        formattedButton += `\n_${footerText}_`;
      }
      ...
      if (result?.nativeFlowMessage?.buttons && result.nativeFlowMessage.buttons.length > 0) {
        formattedButton += '\n\n*Buttons:*\n';
        result.nativeFlowMessage.buttons.forEach((button: any, index: number) => {
          try {
            const buttonParams = button.buttonParamsJson ? JSON.parse(button.buttonParamsJson) : {};
            const buttonText = buttonParams.displayText || buttonParams.name || `Button ${index + 1}`;
            formattedButton += `▶️ ${buttonText}\n`;
          } catch {
            // If JSON parsing fails, use button name or default
            const buttonText = button.name || `Button ${index + 1}`;
            formattedButton += `▶️ ${buttonText}\n`;
          }
        });
      } else if (!bodyText && !headerText && !headerImage && !headerVideo && !headerDocument) {
        // If no content at all, add a default message
        formattedButton += 'Interactive message with buttons';
      }

      return formattedButton;
    }

    if (typeKey === 'buttonResponseMessage' || typeKey === 'nativeFlowResponseMessage') {
      const selectedButtonId = result?.selectedButtonId || result?.selectedId || 'Unknown';
      const responseText = result?.responseText || result?.name || selectedButtonId;

      let formattedResponse = '*Button Response:*\n\n';
      formattedResponse += `_Selected:_ ${responseText}\n`;
      if (selectedButtonId && selectedButtonId !== responseText) {
        formattedResponse += `_ID:_ ${selectedButtonId}\n`;
      }
