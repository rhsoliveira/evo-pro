...
export class BaileysStartupService extends ChannelStartupService {
  ...
  private readonly userDevicesCache = new NodeCache({ stdTTL: 300000, useClones: false });
  ...
  private readonly groupHandler = {
    ...
    'group-participants.update': async (participantsUpdate: BaileysEventMap['group-participants.update']) => {
      ...
      try {
        ...
        const resolvedParticipants = participantsUpdate.participants.map(({ id: participantId }) => {
  ...
  private async sendMessageWithTyping<T = proto.IMessage>(
    ...
    try {
      ...
      let quoted: proto.IWebMessageInfo | Quoted;
  ...
  private async prepareMediaMessage(mediaMessage: MediaMessage, isViewOnce = false) {
    try {
      ...
      const message = { [mediaType]: { ...prepareMedia[mediaType] } };

      if (isViewOnce) {
        message[mediaType].viewOnce = true;
      }

      const messageContent: proto.IMessage = isViewOnce ? { viewOnceMessage: { message } } : message;

      return generateWAMessageFromContent('', messageContent, { userJid: this.instance.wuid });
    } catch (error) {
      this.logger.error(error);
  ...
  public async mediaMessage(data: SendMediaDto, file?: any, isIntegration = false) {
    ...
    const generate = await this.prepareMediaMessage(mediaData, data?.isViewOnce);
  ...
  private toJSONString(button: Button): string {
    ...
    const json = {
      call: () => {
        if (button.type === 'call') {
          return toString({ display_text: button.displayText, phone_number: button.phoneNumber });
        }
      },
      reply: () => toString({ display_text: button.displayText, id: button.id }),
      copy: () => {
        if (button.type === 'copy') {
          return toString({ display_text: button.displayText, copy_code: button.copyText || button.copyCode });
        }
      },
      url: () => {
        if (button.type === 'url') {
          return toString({ display_text: button.displayText, url: button.url, merchant_url: button.url });
        }
      },
      pix: () => {
        if (button.type === 'pix')
  ...
  public async buttonMessage(data: SendButtonsDto) {
    ...
    const hasOtherButtons = data.buttons.some(
      (btn) => btn.type !== 'reply' && !['pix', 'url', 'call', 'code'].includes(btn.type),
    );

    if (hasReplyButtons) {
      if (data.buttons.length > 10) {
        throw new BadRequestException('Maximum of 10 reply buttons allowed');
      }
      if (hasOtherButtons) {
  ...
  private async mountInteractiveMessage(data: SendButtonsDto | SendCarouselDto): Promise<proto.IMessage> {
    const message: proto.IMessage = {
      interactiveMessage: {
        body: {
          text: (() => {
  ...
  public async carouselMessage(data: SendCarouselDto) {
    ...
    const { interactiveMessage: baseInteractiveMessage } = await this.mountInteractiveMessage(data);

    const cardsResolved =
      data.cards && Array.isArray(data.cards)
        ? data.cards.map(async (card) => {
            const base: proto.Message.IInteractiveMessage = {};
            ...
            if (card.footerText && card.footerText.length > 0) {
              base.footer = {
                text: card.footerText,
              };
            }

            if (card.thumbnailUrl && card.thumbnailUrl.length > 0) {
              const generate = await (async () => {
                const { message } = await this.prepareMediaMessage({ mediatype: 'image', media: card.thumbnailUrl });
                return message;
              })();

              base.header = {
                imageMessage: generate.imageMessage,
                hasMediaAttachment: !!generate.imageMessage,
              };
            }
            ...
            return {
              ...base,
              nativeFlowMessage: {
                buttons: card.buttons?.map((value) => {
                  return { name: this.mapType.get(value.type), buttonParamsJson: this.toJSONString(value) };
                }),
              },
    ...
    const cards = await Promise.all(cardsResolved);
  ...
  private generatePaymentReferenceId(): string {
    const characters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789';
    let result = '';
    for (let i = 0; i < 12; i++) {
      result += characters.charAt(Math.floor(Math.random() * characters.length));
  ...
  private generateCustomRetailerId(): string {
    const characters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789';
    let result = '';
    for (let i = 0; i < 10; i++) {
      result += characters.charAt(Math.floor(Math.random() * characters.length));
  ...
  public async paymentOrderMessage(data: SendPaymentOrderDto) {
    ...
    // If there is no referral, validate items and total amount
    if (!hasReferral) {
      if (items.length === 0) {
        throw new BadRequestException('At least one item is required in payment order');
      }
      ...
      const formattedItems = items.map((e) => {
        return {
          ...
          retailer_id: e.retailer_id || this.generateCustomRetailerId(),
        };
      });

      const totalAmount = formattedItems.reduce((acc, item) => acc + item.amount.value * item.quantity, 0);

      if (totalAmount < 1 || totalAmount > 500000) {
        throw new BadRequestException('Total amount must be between 1 and 500000');
      }
    ...
    const cardItem = data.paymentSettings.find((e) => e.type === 'cards');
    const othersPayments = data.paymentSettings.filter((e) => ['boleto'].includes(e.type));

    if (hasReferral && (cardItem?.cards?.enabled || othersPayments.length > 0)) {
      throw new BadRequestException('Referral cannot be used with cards enabled or boleto payment methods');
    }

    const paymentOrderMessage: proto.Message.IInteractiveMessage = {
      nativeFlowMessage: {
        buttons: [
          {
            name: 'review_and_pay',
            buttonParamsJson: JSON.stringify({
              ...
              reference_id: data.referenceId || this.generatePaymentReferenceId(),
              currency: data.currency || 'BRL',
              ...(hasReferral ? { referral: data.referral } : {}),
              ...paramsJson,
            }),
          },
        ],
    ...
    const message: proto.IMessage = {
      interactiveMessage: paymentOrderMessage,
    };
  ...
  public async sendMessageAnyType(data: SendMessageAnyType) {
    ...
    const sender = isWA.jid.toLowerCase();
    return await this.client.sendMessage(sender, data.message);
  }
