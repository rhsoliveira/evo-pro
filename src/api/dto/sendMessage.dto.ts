import { AnyMessageContent, proto, WAPresence } from 'baileys';
...
class BaseButtons extends Metadata {
  title: string;
  description?: string;
  footer?: string;
}
export class SendButtonsDto extends BaseButtons {
  thumbnailUrl?: string;
  buttons: Button[];
}

export class SendCarouselDto extends BaseButtons {
  // header: {
  //   title: string;
  //   subtitle?: string;
  // };
...
export class SendMessageAnyType extends Metadata {
...
// Payment Order Types
export type PaymentOrderPixKeyType = 'PHONE' | 'EMAIL' | 'CPF' | 'CNPJ' | 'EVP';

export class PaymentOrderPixStaticCode {
  key: string;
  key_type: PaymentOrderPixKeyType;
  merchant_name: string;
}

export class PaymentOrderBoleto {
  digitable_line: string;
}

export class PaymentOrderCards {
  enabled: boolean;
}

export type PaymentSettingType = 'pix_static_code' | 'boleto' | 'cards';

export class PaymentSetting {
  type: PaymentSettingType;
  pix_static_code?: PaymentOrderPixStaticCode;
  boleto?: PaymentOrderBoleto;
  cards?: PaymentOrderCards;
}
...
export class SendPaymentOrderDto extends Metadata {
  referenceId?: string;
  referral?: string;
  amount?: number;
  currency?: string;
